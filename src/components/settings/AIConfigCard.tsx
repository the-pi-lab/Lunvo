...
export function AIConfigCard() {
  ...
  const needsBaseUrl =
    ["custom", "ollama", "lmstudio", "openrouter"].includes(profile.provider) || !!profile.baseURL;

  return (
    <div className="bg-surface-container-lowest border border-outline-variant/40 rounded-xl p-6 shadow-sm">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-on-background dark:text-white">
          AI Provider Configuration
        </h2>
        <p className="text-sm text-on-surface-variant mt-1">
          LUNVO is 100% BYOK (Bring Your Own Key). You can use cloud models or run them locally on
          your own machine.
        </p>
      </div>
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-on-surface dark:text-on-surface-variant/40 mb-1">
            AI Provider
          </label>
          <select
            value={profile.provider}
            onChange={handleProviderChange}
            className="w-full rounded-md border border-outline-variant/60 bg-surface-container-lowest px-3 py-2 text-sm text-on-background"
          >
            {PROVIDERS.map((p) => (
        ...
        {needsBaseUrl && (
          <div>
            ...
            <input
              ...
              className="w-full rounded-md border border-outline-variant/60 bg-surface-container-lowest px-3 py-2 text-sm font-mono text-on-background"
            />
          </div>
        )}
        ...
        <div>
          ...
          {profile.provider === "ollama" && localModels.length === 0 && (
            <p className="text-xs text-on-surface-variant mt-1">
              Type the exact model tag (e.g., llama3.2). Start Ollama to auto-detect models.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder={
              profile.provider === "ollama" || profile.provider === "lmstudio"
          ...
          />
          <p className="text-xs text-on-surface-variant mt-1">
            Keys are stored securely in your browser's local storage and are never saved to our
            database.
          </p>
        </div>
