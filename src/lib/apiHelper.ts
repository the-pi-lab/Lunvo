...
import { getActiveProfile, addToVault, setActiveProfileId } from "./ai/profileVault";
...
export function getActiveAIProfile(): AIProfile | null {
  ...
  // 1. Primary: read from Profile Vault (Priority order + active ID)
  const vaultProfile = getActiveProfile();
  if (vaultProfile) return vaultProfile;

  // 2. Fallback: read legacy single-profile key if present
