...
```mermaid
graph LR
  A[Raw Topic] --> B[🕵️ Scout Agent<br/>Finds angle + 3 viral hooks<br/>Data / Story / Contrarian]
  B --> C[✍️ Writer Agent<br/>Injects YOUR Voice DNA<br/>1 idea/line, mobile-first]
  C --> D[🧐 Critic Agent<br/>Audits vs LinkedIn algo<br/>Removes fluff + Virality 1-100]
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/the-pi-lab/Lunvo&env=NEXT_PUBLIC_SUPABASE_URL,NEXT_PUBLIC_SUPABASE_ANON_KEY,GEMINI_API_KEY,GROQ_API_KEY&project-name=lunvo&repository-name=lunvo)
[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/template/github/the-pi-lab/Lunvo)
...
```mermaid
graph TD
  UI[Next 15 App Router] --> Router[router.unified.ts<br/>Single callAI]
  Router --> Providers[Gemini / Groq / NVIDIA / Ollama]
  Router --> Agents[orchestrator.ts<br/>Scout->Writer->Critic]
  Agents --> Prompts[prompts.v3.ts<br/>LINKEDIN_SYSTEM_PROMPT]
  Agents --> DNA[injector.ts<br/>Voice DNA]
  UI --> SB[Supabase SSR<br/>Auth + posts + personas + streak]
  SB --> RLS[RLS policies]
  UI --> RSS[rssService.ts + cacheService.ts]
```
